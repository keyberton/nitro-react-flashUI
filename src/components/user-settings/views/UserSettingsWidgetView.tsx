...
export const UserSettingsWidgetView: FC<UserSettingsWidgetViewProps> = props =>
{
    ...
    return (
        <NitroCardView windowPosition="DWP_TOP_RIGHT" uniqueKey="user-settings" className={ `user-settings-window ${ (selectedSettings === 'audio') && 'sound' } no-resize` } theme="settings">
            ...
            <NitroCardContentView gap={ 2 } className="text-white">
                <Column gap={ 1 }>
                    ...
                    { (selectedSettings === 'audio') &&
                        <Column gap={ 1 }>
                            ...
                            <Flex gap={ 2 }>
                                ...
                                <Flex alignItems="center" gap={ 1 }>
                                    <i className={ (userSettings.volumeSystem > 1) ? 'icon icon-sound-off cursor-pointer' : 'icon icon-sound-off-active cursor-pointer' } onClick={ () => processAction('system_volume_save', 0) } />
                                    <Column gap={ 0 }>
                                    ...
                                    <i className={ (userSettings.volumeSystem < 1) ? 'icon icon-sound-on cursor-pointer' : 'icon icon-sound-on-active cursor-pointer' } onClick={ () => processAction('system_volume_save', 100) } />
                                </Flex>
                            </Flex>
                            <Flex gap={ 2 }>
                                <Text className="w-25">{ LocalizeText('widget.memenu.settings.volume.furni') }</Text>
                                <Flex alignItems="center" gap={ 1 }>
                                    <i className={ (userSettings.volumeFurni > 1) ? 'icon icon-sound-off cursor-pointer' : 'icon icon-sound-off-active cursor-pointer' } onClick={ () => processAction('furni_volume_save', 0) } />
                                    <Column gap={ 0 }>
                                        <Flex className="number-range" />
                                    ...
                                    <i className={ (userSettings.volumeFurni < 1) ? 'icon icon-sound-on cursor-pointer' : 'icon icon-sound-on-active cursor-pointer' } onClick={ () => processAction('furni_volume_save', 100) } />
                                </Flex>
                            </Flex>
                            <Flex gap={ 2 }>
                                <Text className="w-25">{ LocalizeText('widget.memenu.settings.volume.trax') }</Text>
                                <Flex alignItems="center" gap={ 1 }>
                                    <i className={ (userSettings.volumeTrax > 1) ? 'icon icon-sound-off cursor-pointer' : 'icon icon-sound-off-active cursor-pointer' } onClick={ () => processAction('trax_volume_save', 0) } />
                                    <Column gap={ 0 }>
                                        <Flex className="number-range" />
                                    ...
                                    <i className={ (userSettings.volumeTrax < 1) ? 'icon icon-sound-on cursor-pointer' : 'icon icon-sound-on-active cursor-pointer' } onClick={ () => processAction('trax_volume_save', 100) } />
                                </Flex>
                            </Flex>
                        </Column>
                    }
                    <Flex position="relative" alignItems="end">
                        <Button className="px-3 py-1" onClick={ event => processAction('close_view') }>{ LocalizeText('widget.memenu.back') }</Button>
                    </Flex>
                </Column>
